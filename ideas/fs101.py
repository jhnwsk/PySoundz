...
if len(sys.argv) < 2:
	print "Usage: fs-101 frequency waveform harmonic mix filter-attack filter-release"
	sys.exit(1)
...
filter_attack = int(sys.argv[5])

filter_release = int(sys.argv[6])
...
duration_in_samples = samplerate * ((filter_attack + filter_release) / 1000.0) + 1 # Work out the number of samples.
